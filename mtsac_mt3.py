...
from additional_envs import MT3
...
def mtsac_metaworld_mt3(ctxt=None, *, seed, _gpu, n_tasks, timesteps):
    ...
    mt3 = MT3()  # pylint: disable=no-member
    mt3_test = MT3()  # pylint: disable=no-member

    # pylint: disable=missing-return-doc, missing-return-type-doc
    def wrap(env, _):
        return normalize(env, normalize_reward=True)

    train_task_sampler = MetaWorldTaskSampler(mt3,
                                              'train',
                                              wrap,
                                              add_env_onehot=True)
    test_task_sampler = MetaWorldTaskSampler(mt3_test,
                                             'train',
                                             add_env_onehot=True)
    assert n_tasks % 3 == 0
    assert n_tasks <= 500
    mt3_train_envs = train_task_sampler.sample(n_tasks)
    env = mt3_train_envs[0]()
    mt3_test_envs = [env_up() for env_up in test_task_sampler.sample(n_tasks)]
    ...
    mtsac.to()
    trainer.setup(algo=mtsac, env=mt3_train_envs)
